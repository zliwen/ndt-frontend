...
<template>
  <div class="Organization">
    ...
    <div class="gly" v-if="gly">
      <span>您是</span>
      <b>{{userOrgObj.name}}</b>
      <span>的管理员</span>
      <Button type="info" icon="eye" v-if="gly" @click="seyUser()">查看用户</Button>
      <Button v-if="!userOrgObj.isLeaf" type="info" icon="plus-round" @click="addSub()">添加子机构</Button>
    </div>
    <Row style="margin-bottom: 15px;">
      ...
      <Row type="flex" justify="start" :gutter="15">
        ...
        <span style="display: inline-block; width: 100px;">
          <Select v-model="orgSubtypes" placeholder="机构类型">
            <Option v-for="item in orgSubtypesList" :value="item.id" :key="item.id">{{ item.value }}</Option>
          </Select>
        </span>
        </Col>
        <Col>
        ...
        <span style="display: inline-block; width: 240px;">
          <GetCity type="1" @onChange="adminDivisionIdChange" :setValue="adminDivisionId"></GetCity>
      ...
      <Row type="flex" justify="end" :gutter="16">
        ...
        <Button v-if="!gly" type="info" icon="plus-round" @click="add()">添加组织机构</Button>
        </Col>
      </Row>
      </Col>
    </Row>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        tableHeard: [
          ...
          },
          {
            title: '父机构',
            render: (h, params) => {
              return h('span', params.row.parentName);
            }
          },
                  ...
                  on: {
                    click: () => {
                      this.$router.push({
                        ...
                        params: {orgId: params.row.id, orgName: params.row.name,orgSubtype:params.row.orgSubtype}
                      })
                    }
                  }
                }, '查看子机构'),
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.$router.push({
                        name: 'BankingUser',
                        params: {orgId: params.row.id, orgName: params.row.name,orgType: params.row.orgSubtype,userType:this.userType}
                      })
                    }
                  }
                }, '查看用户'),
    ...
    created: function () {
      ...
      if (this.$route.query.orgSubtypes) this.orgSubtypes = parseInt(this.$route.query.orgSubtypes);
      this.getTableData();
      this.$Modal.remove();

      if((this.userType==205||this.userType==206||this.userType==207)&&(this.session.get("roleIds")==6||this.session.get("roleIds")==11||this.session.get("roleIds")==16)){
        this.gly=true;
        orgServer.findById({id:this.session.get("orgId")},{},response=>{
            if(response.errorCode==0)this.userOrgObj=response.data;
    ...
    methods: {
      seyUser(){
        this.$router.push({
          name: 'BankingUser',
          params: {orgId:this.userOrgObj.id, orgName: this.userOrgObj.name,orgType: this.userOrgObj.orgSubtype}
        })
      },
      ...
      getTableData (){
        this.tableData = [];
        this.loading = true;
        this.$router.push('banking?current=' + this.current + "&pageSize=" + this.pageSize
        +"&name="+this.name+"&orgSubtypes="+this.orgSubtypes+"&adminDivisionId="+this.adminDivisionId);
        userServer.findOrgList({
          ...
          orgSubtypes:this.orgSubtypes?[this.orgSubtypes]:null,
      ...
      addSub(){
        let isParent = {isParent: true,
          orgSubtype:this.userOrgObj.orgSubtype,
          parentId: this.userOrgObj.id,parentName:this.userOrgObj.name};
        this.session.set("orgObj", JSON.stringify(isParent));
        this.$router.push({name: "BankingAdd"})
      },
      editInit(data){
...
<style lang='less'>
  .Organization {
    .gly{ padding-bottom: 30px;
      >span{ font-size: 18px;}
      >b{color: #00a0e9;font-size: 18px;}
      button{ float: right; margin-left: 20px;}
    }
  }
</style>
