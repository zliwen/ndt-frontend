...
<template>
  <div class="bankingEdit">
    <Breadcrumb style=" margin-bottom: 15px;">
      ...
      <BreadcrumbItem v-if="isParent&&isParent!=3"
                      :href="'/user/banking/children/'+formOrgItem.parentId+'/'+formOrgItem.parentName+'/'+parentOrgSubtype">
        {{formOrgItem.parentName}}
      </BreadcrumbItem>
    ...
    <div class="contentBox">
      <Form ref="formOrgItem" :model="formOrgItem" :rules="ruleCustom" :label-width="120">
        ...
        <FormItem label="金融机构类型：" prop="orgSubtype" v-if="!isParent||isParent==3">
          <RadioGroup v-model="formOrgItem.orgSubtype" @on-change="orgSubtypeChange">
            <Radio v-for="item in orgSubtypesList" :key="item.id" :label="item.value"></Radio>
          </RadioGroup>
        </FormItem>

        <FormItem label="父级机构：" prop="parentId" v-if="!isParent||isParent==3">
          <Select v-model="formOrgItem.parentId" clearable @on-change="findById(formOrgItem.parentId)">
        ...
        <FormItem label="机构所在区域：" prop="adminDivisionId">
          <GetCity type="1" :set-value="formOrgItem.adminDivisionId" @onChange="adminDivisionIdChange"></GetCity>
        </FormItem>
        <FormItem label="机构管辖范围：" prop="divisions" style="margin-right:-40px;">
          <GetCitys type="1"  :set-values="formOrgItem.divisionss" @onChange="divisionsChange"></GetCitys>
        </FormItem>

        <div>
          <span v-if="formOrgItem.parentId">
             <span style="margin-left: 30px;">logo是否沿用上级</span>
            <i-switch v-model="isLogoParent" style="display: inline-block;"></i-switch>
          </span>
          <div style="margin-top: 20px;">
            <FormItem class="logo" label="机构logo：" prop="logo">
            ...
            <FormItem class="logo" label="产品logo：" prop="logo">
              <img class="imgs" v-if="isLogoParent" :src="parentObj.squareLogo" alt="">
        ...
        <FormItem label="机构描述：" prop="remark">
          ...
          <Input v-if="!isRemarkParent||formOrgItem.parentId" :rows="5"
  ...
  export default {
    ...
    created: function () {
      ...
      if (data.id) {
        ...
        if (this.formOrgItem.parentId) this.findById(this.formOrgItem.parentId);
      ...
      if(this.isParent) this.findById(data.parentId||this.session.get("orgId"),true);
    },
    methods: {
      ...
      findById(id,k){
        ...
        orgServer.findById({id: id}, {}, response => {
          if(response.errorCode == 0){
            this.parentOrgSubtype = response.data.orgSubtype;
            this.formOrgItem.parentId = response.data.id;
            this.parentObj=response.data;
          }
        }, err => {
        })
      },
      ...
      addSave () {
        this.$refs.formOrgItem.validate((valid) => {
          ...
          var json = {
            ...
            "orgType": 902,
            orgSubtype: this.isParent ? this.parentOrgSubtype : orgSubtype,
            ...
            "squareLogo": this.isLogoParent ? "" : this.formOrgItem.squareLogo,
...
<style lang='less'>
  ...
  .logo{
    ...
    .imgl{ height: 100px; width: 240px; display: inline-block; border:1px solid #dfdfdf;}
    .imgs{ height: 100px; width: 100px; display: inline-block; border:1px solid #dfdfdf;}
  }
