        ...
        <div  class="edit-eara" style="width:910px; margin: 0 auto">
            <Form  v-show="true" ref="formData" :model="formData"  inline :rules="ruleCustom">
                    ...
                    <FormItem  label="地区" required label-position="right" :label-width='60' class="ivu-form-item-required">
                        <GetCity ref='getCity' :disabled="true" style="width:400px;" :setValue="initValue" @onChange="adminDivisionIdChange"></GetCity>
                    </FormItem>
                    <FormItem label="优先级" prop="importance" label-position="right" :label-width='60'>
                        <Select :disabled='readOnly' v-model="formData.importance"  placeholder="优先级"   style=" width: 338px;">
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                ...
                ruleCustom: {
                    ...
                    // ],
                    importance:[
                        { required: true, message: '请选择村务信息优先级', trigger: 'blur,change' }
                    ],
                    // divisions:[
        ...
        created: function () {
            ...
            let obj = JSON.parse(session.get('villageObj'));
            //session.remove("villageObj");
            ...
            if(obj.type == "new"){
                this.readOnly = false;
                this.initValue = JSON.parse(this.session.get("divisions"))[0];
                this.formData.divisions = JSON.parse(this.session.get("divisions"));
            }
            else{
        ...
        methods: {
            ...
            upImgSuccess(response){
                // if(response.errorCode==0){
                //     this.formData.linkImageUrl = response.url;
