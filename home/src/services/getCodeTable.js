...
window.codeTableObj = {
  ...
  getAllCodeTable(){
    if ((new Date()) - 0 - window.codeTableObj.time < 2000) return;
    window.codeTableObj.time = (new Date()) - 0;
    dictServer.list({params: {params3: 1, params4: 600}, data: {list: this.getKeyList()}}, response => {
  ...
  getCodeName(key){
    if (!key) return;
    let nKey = "";
    if (!codeTableObj.codeTableList.length && session.get("codeTableList")) codeTableObj.codeTableList = (JSON.parse(session.get("codeTableList"))).list;
    codeTableObj.codeTableList.forEach((item) => {
      if (item.id == key) nKey = item.value
    });
    if (!nKey) codeTableObj.getAllCodeTable();
    return nKey || key;
  },
  getCodeTable (key, callBack) {
    let co = codeTableObj.codeTableObj[key];
    if (!co && session.get("codeTableObj")) {
